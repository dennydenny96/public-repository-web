...
var App = function () {
    ...
    var handleIframeContent = function () {
        ...
        if ($tabs.is(":visible")) {
            height = height - $tabs.outerHeight() - 10;
        }
    ...
    var handleBootstrapConfirmation = function () {
        ...
        $('[data-toggle=confirmation]').confirmation({
            container: 'body',
            btnOkClass: 'btn btn-sm is-green',
            btnCancelClass: 'btn btn-sm is-red'
        });
    };
  ...
  context = (function () {
    ...
    function buildMenu(data, id, subMenu) {
        var subClass = (subMenu) ? ' dropdown-context-sub' : '',
            ...
            $menu = $('<div class="dropdown-menu dropdown-context' + subClass + compressed + '" id="dropdown-' + id + '"></div>');
        ...
        for (var i = 0; i < data.length; i++) {
            if (typeof data[i].divider !== 'undefined') {
                ...
                if (typeof data[i].subMenu !== 'undefined') {
                    ...
                    sub_menu += '"><a class="dropdown-item" tabindex="-1" href="' + data[i].href + '">' + data[i].text + '</a></li>'
                    $sub = (sub_menu);
                } else {
                    var element = '<li';
                    element += (addDynamicTag) ? ' class="dynamic-menu-item"' : '';
                    element += '><a class="dropdown-item" tabindex="-1" href="' + data[i].href + '"' + linkTarget + '>';
  ...
  */
  ...
  function canRemoveTab(pageId) {
    return findTabTitle(pageId).find('.typcn.typcn-times').length > 0;
  }
  ...
  var addTabs = function (options) {
    ...
    if (findTabPanel(pageId) == null) {
        ...
        if (options.close) {
            var $i = $("<i class='typcn typcn-times page_tab_close' style='cursor: pointer' onclick='closeTab(this);'></i>").attr(pageIdField, pageId).appendTo($title);
        }
  ...
  */
  ...
  var closeOtherTabs = function (isAll) {
    if (isAll) {
        $('.page-tabs-content').children("[" + pageIdField + "]").find('.typcn.typcn-times').parents('a').each(function () {
            var $a = $(this);
        ...
        $('.page-tabs-content').children("[" + pageIdField + "]").find('.typcn.typcn-times').parents('a').not(".active").each(function () {
            var $a = $(this);
            var pageId = getPageId($a);
            closeTabOnly(pageId);
            ...
            $.each(data, function (i, item) {
              ...
              //   $a.append($title);
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    var pullIcon = $('<i class="fa fa-angle-right pull-right"></i>');
                  ...
                  $a.append($title);
                    ...
                    else if (item.targetType != null && item.targetType === "ajax") {
                        $a.attr("href", item.url);
                        $a.addClass("ajaxify");
                    }
                    else if (item.targetType != null && item.targetType === "iframe-tab") {
                        var _title = (item.text.indexOf("'") > 0)? item.text.replace("'","\\'"): item.text;
                        var split = item.url.split('~');
                        if(split.length > 1){
                          var param = split[1].split('/?');
                          item.url = split[0] + "/?" + param[1] + "&" + param[0];
                        }
                        var href = 'addTabs({id:\'' + item.id + '\',title:\'' + _title + '\',close:true,url: \'' + item.url + '\'});';
                        $a.attr('onclick', href);
                    }
